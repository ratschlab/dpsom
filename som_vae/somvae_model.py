...
class SOMVAE:
    ...
    def prediction_input(self):
        pred_input = tf.placeholder(tf.int64, shape=[None], name="pred_input")
        return pred_input
    ...
    def z_e(self):
        ...
        if not self.mnist:
            with tf.variable_scope("encoder"):
                h_1 = tf.keras.layers.Dense(256, activation="relu")(self.x) 
    ...
    def loss_reconstruction(self):
        ...
        loss_rec_mse_zq = tf.losses.mean_squared_error(self.x, self.reconstruction_q)
        loss_rec_mse_ze = tf.losses.mean_squared_error(self.x, self.reconstruction_e)
